...
export function ProductFormDialog({
  ...
  async function applyBarcodeScan(code: string) {
    ...
    if (result.source === "local") {
      toast.error(
        `${result.name} is already in your catalog${result.barcode ? ` (${result.barcode})` : ""}.`
      );
      return;
    }
    ...
    if (result.source === "openfoodfacts") {
      ...
      if (result.categoryName) setValue("categoryName", result.categoryName);
      toast.success("Product details filled from barcode lookup.");
      return;
    }

    setValue("barcode", result.barcode || code);
    toast.message("Barcode saved. Enter the product details manually.");
  }
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {isEdit
                ...
                : "Add a new product or scan a barcode to auto-fill details."}
            </DialogDescription>
          </DialogHeader>
